...
import { getRightsDraft, submitDetailsStep } from "../actions";
...
export async function generateMetadata({
  ...
  const title = stepTitle(buildRightsWizardLabels(locale).detailsHeading, journey);
  return buildMetadata({ locale, title, description: journey, path: "/privacy/your-data/details" });
}

export default async function RightsDetailsPage({
  params,
  searchParams,
}: {
  params: Promise<{ lang: string }>;
  ...
  const backHref = localeHref(
    locale,
    returnTo === "check" ? "/privacy/your-data/check" : "/privacy/your-data/email"
  );
  const progress =
    returnTo === "check"
      ? undefined
      : formatStepOf(chrome.stepOf, RIGHTS_STEPS.indexOf("details") + 1, RIGHTS_STEPS.length);
    ...
    <>
      ...
      <div className="wrap max-w-[var(--measure)] py-10">
        <div className="flex flex-col gap-6">
          <QuestionStepForm
            action={submitDetailsStep.bind(null, locale, returnTo)}
            ...
            field={{
              ...
              name: "details",
              as: "textarea",
              label: wizard.detailsHeading,
              hint: wizard.detailsHint,
