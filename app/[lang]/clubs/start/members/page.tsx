...
import { getStartClubDraft, submitMembersStep } from "../actions";
...
export async function generateMetadata({
  ...
  const title = stepTitle(buildStartClubWizardLabels(locale).membersHeading, journey);
  return buildMetadata({ locale, title, description: journey, path: "/clubs/start/members" });
}

export default async function StartClubMembersPage({
  params,
  searchParams,
}: {
  params: Promise<{ lang: string }>;
  ...
  return (
    <>
      <PageHeader title={wizard.membersHeading} />
      <div className="wrap max-w-[var(--measure)] py-10">
        <div className="flex flex-col gap-6">
          <StepNav backHref={backHref} backLabel={chrome.back} progressText={progress} />
          <QuestionStepForm
            action={submitMembersStep.bind(null, locale, returnTo)}
            ...
            field={{
              name: "members",
              label: wizard.fieldLabels.members,
              hint: wizard.membersHint,
              optionalLabel: dict.actions.optional,
              defaultValue: draft.members,
            }}
          />
        </div>
      </div>
