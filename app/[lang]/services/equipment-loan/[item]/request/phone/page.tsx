...
import { getLoanDraft, submitPhoneStep } from "../actions";
...
export async function generateMetadata({
  ...
  return buildMetadata({
    locale,
    title: stepTitle(buildLoanWizardLabels(locale, item).phone.question, item.name[locale]),
    description: item.name[locale],
    path: `/services/equipment-loan/${itemKey}/request/phone`,
  });
}

export default async function LoanRequestPhonePage({
  params,
  searchParams,
}: {
  params: Promise<{ lang: string; item: string }>;
  ...
  const backHref = localeHref(locale, returnTo === "check" ? `${base}/check` : `${base}/email`);
  const progress =
    returnTo === "check"
      ? undefined
      : formatStepOf(labels.common.stepOf, LOAN_STEPS.indexOf("phone") + 1, LOAN_STEPS.length);

  return (
    <>
      <PageHeader title={labels.phone.question} />
      <div className="wrap max-w-[var(--measure)] py-10">
        <div className="flex flex-col gap-6">
          <StepNav backHref={backHref} backLabel={labels.common.back} progressText={progress} />
          <QuestionStepForm
            action={submitPhoneStep.bind(null, locale, itemKey, returnTo, labels)}
