...
import { getLoanDraft, submitEmailStep } from "../actions";
...
export async function generateMetadata({
  ...
  return buildMetadata({
    locale,
    title: stepTitle(buildLoanWizardLabels(locale, item).email.question, item.name[locale]),
    description: item.name[locale],
    path: `/services/equipment-loan/${itemKey}/request/email`,
  });
}

export default async function LoanRequestEmailPage({
  params,
  searchParams,
}: {
  params: Promise<{ lang: string; item: string }>;
  ...
  return (
    <>
      <PageHeader title={labels.email.question} />
      <div className="wrap max-w-[var(--measure)] py-10">
        <div className="flex flex-col gap-6">
          <StepNav backHref={backHref} backLabel={labels.common.back} progressText={progress} />
          <QuestionStepForm
            action={submitEmailStep.bind(null, locale, itemKey, returnTo, labels)}
            ...
            field={{
              ...
              defaultValue: draft.studentEmail,
              autoComplete: "email",
            }}
          />
        </div>
