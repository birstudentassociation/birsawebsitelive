...
## Emergency alerts (committed content)

The site-wide emergency banner is driven by `content/emergency/active.ts`. It names one of the
guides in `content/emergency/scenarios.ts`, with an issue time and optional banner text and
updates. Raising, updating or ending an alert is a commit and a deploy (about a minute), so
every page stays fully static. `docs/EDITING.md` has the steps.
